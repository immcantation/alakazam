---
title: "Clonal diversity statistics"
...
--------------------------------------------------------------------------------
The function `bootstrapDiversity` performs uniform resampling, with replacement,
of the input sequences and recalculates the clone size distribution, and 
diversity, with each bootstrap realization. Diversity (D) is calculated over a
range of diversity orders (q) to generate a smooth curve.
...
sample_div <- bootstrapDiversity(df, "SAMPLE", min_q=0, max_q=32, step_q=0.05, 
                                 ci=0.95, nboot=2000)
...
isotype_div <- bootstrapDiversity(df, "ISOTYPE", min_n=30, min_q=0, max_q=32, 
...
the same manner as `bootstrapDiversity`, but only for a single diversity order. 
