...
properties of Ig and TCR amino acid sequence. Of particular interest is the analysis of 
CDR3 properties, which this vignette will demonstration. However, the same process
can be applies to other regions simply by altering the sequence data column used.
...
--------------------------------------------------------------------------------
Multiple amino acid chemical properties can be obtained with the function 
...
Translate of the DNA sequences to amino acid sequences is accomplished by 
...
codon (the conserved resiudes) prior to analysis. The prefix `CDR3` is added
...
grand average of hydrophobicity index (`gravy`) and normalized net charge by
...
required this may be accomplished using one of the work functions:
...
Input to each function must be vector of amino acid sequences.

```{r, eval=TRUE, warning=FALSE, message=FALSE}
# Translate junction DNA sequences to amino acids and trim first and last codons
cdr3 <- translateDNA(db$JUNCTION[1:5], trim=TRUE)
...
The following references were used for the default physiochemical scales:
