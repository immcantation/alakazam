...
#'           }
#' 
#' @seealso  \code{\link{getPathLengths}} and \code{\link{plotSubtrees}}.
#' @examples
#' # Define simple graph
#' library(igraph)
...
summarizeSubtrees <- function(graph, root="Germline", fields=NULL) {
    ...
    for (f in fields) {
        node_df[, f] <- get.vertex.attribute(graph, name=f)
    }
    
    # Get edges
    edges <- igraph::get.edgelist(graph)
    # Get unweighted paths
    paths_step <- suppressWarnings(igraph::shortest.paths(graph, mode="out", algorithm="unweighted"))
    paths_step[!is.finite(paths_step)] <- NA
    # Get weighted paths
    paths_length <- igraph::shortest.paths(graph, mode="out", algorithm="dijkstra")
...
#' # Define simple graph
#' library(igraph)
#' graph <- graph(c(1,2,2,3,2,4,3,5,3,6,6,7,7,8,7,9), directed=TRUE)
#' V(graph)$name <- c("Germline", "Inferred1", "Seq1", "Seq2", "Seq3", "Seq4", "Inferred2", "Seq5", "Seq6")
...
tableEdges <- function(graph, field, indirect=FALSE, exclude=c("Germline", NA)) {
    ...
    .getSingleVertex <- function(x) {
        if (length(x) == 1) { 
            get.vertex.attribute(graph, name=field, index=x[1]) 
        } else { 
            NULL 
        }
    }
    ...
    if (indirect) {
        ...
        if (!is.null(exclude)) {
            f <- get.vertex.attribute(graph, name=field)
        ...
        for (i in keep_idx) { 
            ...
            parent <- get.vertex.attribute(graph, name=field, index=i)
            
            # Get indirect child node annotations
            step_list <- suppressWarnings(get.shortest.paths(graph, V(graph)[i], mode="out", weights=NA, output="vpath"))
        ...
        # Get adjacency list
        edge_mat <- get.edgelist(graph, names=FALSE)
        edge_mat <- get.vertex.attribute(graph, name=field, index=edge_mat)
    ...
    # Count edges
    edge_tab <- ddply(edge_df, .(parent, child), summarize, count=length(parent))
...
permuteLabels <- function(graph, field, exclude=c("Germline", NA)) {
    ...
    labels <- get.vertex.attribute(graph, name=field)
    ...
    s <- sample(i)
    perm <- set.vertex.attribute(graph, name=field, index=i, value=labels[s])
...
#'           
#' @seealso  Uses \code{\link{getFounder}} and \code{\link{getPathLengths}}. Return object
#'           can be plotted with \code{\link{plotFounderTest}}.
#' @examples
...
testFounders <- function(graphs, field, root="Germline", exclude=c("Germline", NA), 
    ...
    resolveFounders <- function(df) {
        ...
        df <- ddply(df, .(graph), subset, !duplicated(label))
        # Discard unresolved ties
        df <- ddply(df, .(graph), subset, length(label) == 1)
    ...
    # Get founders
    obs_df <- ldply(graphs, getFounder, path="distance", field=field, 
    ...
    obs_sum <- ddply(obs_df, .(label), summarize, count=length(label))
    ...
    perm_list <- list()
    for (i in 1:nperm) {
        tmp_list <- llply(graphs, permuteLabels, field=field, exclude=exclude)
        tmp_df <- ldply(tmp_list, getFounder, path="distance", field=field, 
                        exclude=exclude, .id="graph")
        tmp_df <- tmp_df[, c("graph", field)]
        names(tmp_df) <- c("graph", "label")
        tmp_df <- resolveFounders(tmp_df)
        tmp_sum <- ddply(tmp_df, .(label), summarize, count=length(label))
        tmp_sum$perm <- i
        perm_list[[i]] <- tmp_sum
        setTxtProgressBar(pb, i)
    }
    cat("\n")
    perm_sum <- rbind.fill(perm_list)
...
#' 
#' @seealso  Uses \code{\link{tableEdges}} and \code{\link{permuteLabels}}. Return object
#'           can be plotted with \code{\link{plotEdgeTest}}.
#' @examples
#' # Define simple graph
#' library(igraph)
#' graph <- graph(c(1,2,2,3,2,4,3,5,3,6), directed=TRUE)
...
testEdges <- function(graphs, field, exclude=c("Germline", NA), nperm=2000) {
    ...
    obs_df <- ldply(graphs, tableEdges, field=field, exclude=exclude)
    obs_sum <- ddply(obs_df, .(parent, child), summarize, count=sum(count, na.rm=TRUE))
    ...
    for (i in 1:nperm) {
        tmp_list <- llply(graphs, permuteLabels, field=field, exclude=exclude)
        tmp_df <- ldply(tmp_list, tableEdges, field=field, exclude=exclude)
        tmp_sum <- ddply(tmp_df, .(parent, child), summarize, count=sum(count, na.rm=TRUE))
        tmp_sum$perm <- i
        perm_list[[i]] <- tmp_sum
        setTxtProgressBar(pb, i)
    }
...
#' @return   NULL
#' 
#' @seealso  \code{\link{testFounders}}.
#' @examples
#' # Define simple graph
#' graph <- graph(c(1,2,2,3,2,4,3,5,3,6), directed=TRUE)
#' library(igraph)
...
#' 
#' @seealso  \code{\link{summarizeSubtrees}}.
#' @examples
#' # Define simple set of graphs
#' library(igraph)
#' graph <- graph(c(1,2,2,3,2,4,3,5,3,6), directed=TRUE)
...
plotSubtrees <- function(graphs, field, root="Germline", exclude=c("Germline", NA), style="box") {
    ...
    sum_df <- plyr::ddply(sum_df, .(tree_identifier), mutate, 
