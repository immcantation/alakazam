...
(columns) to be present in the Change-O file: `SEQUENCE_ID`, `SEQUENCE_GAP`, 
`CLONE`, `GERMLINE_GAP_D_MASK` `V_CALL`, `J_CALL`, and `JUNCTION_GAP_LENGTH`.
...
and trailing ends of each sequence. The function `prepChangeoClone` is a wrapper
...
Two arguments to `prepChangeoClone` control which annotations are retained
...
clone <- prepChangeoClone(sub_df, text_fields=c("SAMPLE", "ISOTYPE"), 
                          num_fields="DUPCOUNT")
...
output by `prepChangeoClone` and returns an igraph `graph` object. The igraph
...
data.frame(CLONE=graph$clone,
           JUNCTION_GAP_LENGTH=graph$junc_len,
           V_GENE=graph$v_gene,
           J_GENE=graph$j_gene)
...
igraph package. A tree layout can be built using the `layout.reingold.tilford` 
layout with assignment of the root position to the germline sequence, which is 
named "Germline" in the object returned by `buildPhylipLineage`. 

```{r, eval=TRUE, fig.align="center"}
# Plot graph with tree layout
ly <- layout.reingold.tilford(graph, root="Germline", circular=F, flip.y=T)
plot(graph, layout=ly, edge.arrow.mode=0)
```

The default edge and vertex labels are edge weights and sequence identifiers, 
respectively. The default attributes are not very pretty. We can modify the 
graphical parameter in the usual igraph ways.
...
plot(graph, layout=ly, edge.arrow.mode=0, vertex.label.color="black", 
     vertex.size=50)
...
dnapars_exec <- "~/apps/phylip-3.69/dnapars"
# Preprocess clones
clone_list <- dlply(df, "CLONE", prepChangeoClone, 
                    text_fields=c("SAMPLE", "ISOTYPE"), 
                    num_fields="DUPCOUNT")
# Build lineages
graph_list <- llply(clone_list, buildPhylipLineage, 
...
graph_list[sapply(graph_list, is.null)] <- NULL
