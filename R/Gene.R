...
countGenes <- function(data, gene, groups = NULL, copy = NULL, clone = NULL, fill = FALSE, first = TRUE, collapse = TRUE,
    ...
    if (remove_na) {
        ...
        if (any(bool_na)) {
            if (!all(bool_na)) {
                msg <- paste0(
                    "NA(s) found in ", sum(bool_na), " row(s) of the ", gene,
    ...
    if (mode != "asis") {
        ...
        data[[gene]] <- gene_func(data[[gene]], first = first, collapse = collapse)
    }
    ...
    if (cell_id %in% names(data)) {
        ...
        if (is.null(copy) & is.null(clone)) {
            ...
            locus_tab <- data %>%
                dplyr::group_by(!!!rlang::syms(unique(c(groups, "locus")))) %>%
                dplyr::summarize(locus_count = n(), .groups = "drop")
            gene_tab_count <- data %>%
                dplyr::group_by(!!!rlang::syms(unique(c(groups, "locus", gene)))) %>%
                dplyr::summarize(seq_count = n(), .groups = "drop")
            if (nrow(locus_tab) > 1) {
                gene_tab <- gene_tab_count %>%
                    dplyr::left_join(locus_tab, by = unique(c(groups, "locus"))) %>%
                    dplyr::mutate(seq_freq = !!rlang::sym("seq_count") / !!rlang::sym("locus_count")) %>%
                    dplyr::arrange(desc(!!rlang::sym("seq_count")))
            } else {
                gene_tab <- gene_tab_count %>%
                    dplyr::mutate(seq_freq = !!rlang::sym("seq_count") / locus_tab$locus_count) %>%
                    dplyr::arrange(desc(!!rlang::sym("seq_count")))
            }
            ...
            # Find count of genes within each clone and keep first with maximum count
            clone_gene_tab <- data %>%
                dplyr::group_by(!!!rlang::syms(unique(c(groups, "locus", clone, gene)))) %>%
                dplyr::mutate(clone_gene_count = n()) %>%
                dplyr::ungroup() %>%
                dplyr::group_by(!!!rlang::syms(unique(c(groups, "locus", clone)))) %>%
                dplyr::slice(which.max(!!rlang::sym("clone_gene_count"))) %>%
                dplyr::ungroup()
            ...
            if (nrow(locus_tab) > 1) {
                gene_tab <- gene_tab_count %>%
                    ...
                    dplyr::arrange(desc(!!rlang::sym("clone_count")))
            } else {
                gene_tab <- gene_tab_count %>%
                    dplyr::mutate(clone_freq = !!rlang::sym("clone_count") / locus_tab$locus_clone_count) %>%
                    dplyr::arrange(desc(!!rlang::sym("clone_count")))
            }
        } else {
            if (!is.null(clone) & !is.null(copy)) {
                warning(
                    "Specifying both 'copy' and 'clone' columns is not meaningful. ",
            ...
            if (nrow(locus_tab) > 1) {
                gene_tab <- gene_tab_count %>%
                    dplyr::left_join(locus_tab, by = unique(c(groups, "locus"))) %>%
                    dplyr::mutate(
                        seq_freq = !!rlang::sym("seq_count") / !!rlang::sym("locus_count"),
                        copy_freq = !!rlang::sym("copy_count") / !!rlang::sym("locus_copy_count")
                    ) %>%
                    dplyr::arrange(desc(!!rlang::sym("copy_count")))
            } else {
                gene_tab <- gene_tab_count %>%
                    dplyr::mutate(
                        seq_freq = !!rlang::sym("seq_count") / locus_tab$locus_count,
                        copy_freq = !!rlang::sym("copy_count") / locus_tab$locus_copy_count
                    ) %>%
                    dplyr::arrange(desc(!!rlang::sym("copy_count")))
            }
        }
...
getSegment <- function(segment_call, segment_regex, first = TRUE, collapse = TRUE,
    ...
    if (omit_nl) {
        ...
        # non-localized regex
        nl_regex <- paste0(
            "(IG[HKL]|TR[ABDG])[VDJADEGMC][0-9]+-NL[0-9]([-/\\w]*[-\\*][\\.\\w]+)*(",
            sep, "|$)"
        )
...
getAllele <- function(segment_call, first = TRUE, collapse = TRUE,
                      strip_d = TRUE, omit_nl = FALSE, sep = ",") {
    allele_regex <- "((IG[HKL]|TR[ABDG])[VDJADEGMC][A-R0-9\\(\\)]*[-/\\w]*[-\\*]*[\\.\\w]+)"
    r <- getSegment(segment_call, allele_regex,
...
getGene <- function(segment_call, first = TRUE, collapse = TRUE,
    ...
    r <- getSegment(segment_call, gene_regex,
        first = first, collapse = collapse,
        strip_d = strip_d, omit_nl = omit_nl, sep = sep
    )
...
getFamily <- function(segment_call, first = TRUE, collapse = TRUE,
    ...
    r <- getSegment(segment_call, family_regex,
        first = first, collapse = collapse,
        strip_d = strip_d, omit_nl = omit_nl, sep = sep
    )
